...
easy-word 作为鸿蒙 NEXT 原生应用，数据层面临两个核心问题：单词数据需要同时展示 IPA 音标和简易拼读标注，多设备之间需要保持学习进度同步。这篇文章梳理这两个问题的实现方案。
...
1. **信任框架的冲突解决**：不做应用层合并，依赖 ArkData 的 LWW。对于单词学习进度这种场景，少量数据被覆盖是可以接受的。
2. **本地数据库做事实源**：所有业务逻辑都基于本地 RDB 查询，同步是异步的后台操作。即使断网，应用也能正常使用。
3. **COALESCE 保 synced_at**：用一条 SQL 解决了 upsert 清空同步标记的问题，比应用层判断更可靠。

目前这套方案在两台设备间的同步延迟在 1-3 秒内，学习进度基本无感。如果未来需要支持多于两台设备的同步，你会选择引入版本向量（Vector Clock）来处理更复杂的冲突场景吗？
